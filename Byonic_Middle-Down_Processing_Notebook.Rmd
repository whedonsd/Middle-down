...
setwd(directory)
TMT_ETD_mz_values <- cfg$TMT_ETD_mz_values
TMT_HCD_mz_values <- cfg$TMT_HCD_mz_values
TMT126 <- cfg$TMT126
TMT127 <- cfg$TMT127
TMT128 <- cfg$TMT128
TMT129 <- cfg$TMT129
TMT130 <- cfg$TMT130
TMT131 <- cfg$TMT131
max_mods <- cfg$max_mods
PEP <- cfg$PEP
min_delta_mod <- cfg$min_delta_mod
ppm_tolerance <- cfg$ppm_tolerance
aa_tag <- cfg$aa_tag
amino_acid_masses <- middleDownR:::amino_acid_masses
...
# helper functions now provided by middleDownR
...
experiment1_list <- process_Byonic_list(experiment1_list)

experiment2_list <- process_Byonic_list(experiment2_list)

experiment3_list <- process_Byonic_list(experiment3_list)
...
experiment1_mz_list <- process_scans_from_mzMLgz(experiment1_scan_list, experiment1_mz)

experiment2_mz <- openMSfile("C:/path_to_your_data/file2.mzML.gz")

experiment2_mz_list <- process_scans_from_mzMLgz(experiment2_scan_list, experiment2_mz)

experiment3_mz <- openMSfile("C:/path_to_your_data/file3.mzML.gz")

experiment3_mz_list <- process_scans_from_mzMLgz(experiment3_scan_list, experiment3_mz)
```
...
clusterExport(cl, varlist = c("ppm_tolerance",
								              "amino_acid_masses",
								              "atomic_masses",
...
clusterEvalQ(cl, {
  library(middleDownR) # gives access to exported fns
  library(dplyr)  
  library(purrr)
  library(tidyr)
  library(stringr)
...
# provided by middleDownR::process_chunk
...
```{r}
# Assumes that matches_data_df_w_mods already exists and has a 'modifications' column
# Define fill_modifications_direct function to work directly on the dataframe
# provided by middleDownR::fill_modifications_direct
...
clusterExport(cl, varlist = c("max_mods"))
clusterEvalQ(cl, {
  library(middleDownR)
  library(dplyr)
  library(purrr)
...
# Assumes that max_mods is set to 7, and in some searches no species with 7 mods will survive the preceding evaluation criteria; in these cases auto-generating columns for 6 and 7 mods prevents subsequent code from producing an error
# provided by middleDownR::add_column_if_not_exists
...
```{r}
# Assumes max_mods is already defined as the maximum number of modifications you expect per peptide
...
# Assumes you have a dataframe `matches_data_df_w_mods` with peptides, modification positions, and masses
# Update the function to directly modify the dataframe
# provided by middleDownR::apply_nearest_function_directly
...
find_nearest_results <- parLapply(cl, grouped_df_list, function(group) {
  apply_nearest_function_directly(group, max_mods)
})
...
# provided by middleDownR::assemble_ions_df
# provided by middleDownR::check_unambiguity_before
# provided by middleDownR::check_unambiguity_after
...
# Step 3: Export necessary objects and functions to each cluster node
clusterExport(cl, varlist = character(0))
clusterEvalQ(cl, {
  library(middleDownR)
  library(dplyr)
  library(purrr)
...
```{r}
# Helper functions provided by middleDownR::conditional_merge_hit_df
# Join hit dataframes
...
# Export necessary objects and functions
clusterExport(cl, varlist = c("grouped_ion_intensity_list"))
clusterEvalQ(cl, {
  library(middleDownR)
  library(dplyr)
  library(purrr)
...
# provided by middleDownR::retrieve_value
...
# provided by middleDownR::check_against_reference
...
# provided by middleDownR::find_intensities_for_scan
...
# provided by middleDownR::TMT_check
